...
__from General Assembly class instructions__
...
```
NOTE:
Make sure you are in your App project directory before you run this command
...
__Do not use any special characters!__ Special characters can complicate the process when configuring your Atlas database with Heroku. upeer and lower letters and numbers only !!!
...
Add and commit all changes we've made to Book-e JSON. It is a good idea to push to your git repo as well. 
IMPORTANT if you used the solution branch, make sure you're making the commit to the solution branch.
...
Set the URI you just copied as an environment variable called DB_URL using heroku config:set, filling in the <USERNAME> and <PASSWORD> you created on the "Users" page. Run the following in your terminal (in your project's folder)...
  copy from after $  heroku thru URL=" paste your link - insert your username / password without the <>'s and put closing " at end
...
Your database name will be included in the URI you copied from your Atlas database. You will need to manually add the USERNAME and PASSWORD that you created in Step 10. __DONT FORGET TO PUT QUOTES AROUND THE URL PART__
...
```
Seed your Mongo DB Atlas database by running the command:
```
